...
Detects various CoinJoin protocols: Whirlpool, Wasabi, JoinMarket, PayJoin.
"""
import logging
...
class CoinJoinDetector:
    ...
    def analyze_transaction(self, tx: Dict) -> DetectionResult:
        ...
        # Wasabi detection
        wasabi_result = self._detect_wasabi(stats)
        results.append(("wasabi", wasabi_result))
    ...
    def _detect_wasabi(self, stats: TransactionStats) -> Tuple[float, float, List[str], List[str], Dict]:
        """
        Detect Wasabi CoinJoin pattern.
        Characteristics:
